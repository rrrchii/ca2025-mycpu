...
// 為了方便使用，把一些常用的instruction type用alias表示
object Instructions {
...
class InstructionDecode extends Module {
  ...
  // R-type : add/sub/and/or/xor/sll/slr/sra/slt/sltu 不需要 immediate
  when(needsImmediate) {
  ...
  // [31:25]：imm[11:5]（立即數高 7 位）、[11:7]：imm[4:0]（立即數低 5 位）
  // 合在一起才是正確的立即數
  val immS = Cat(
